...
async function loadStats() {
    try {
        ...
        document.getElementById('statMemory').textContent = 'N/A';
    }
}

async function loadAPIs() {
    try {
        // ✅ FIXED: Removed ':1' - using correct endpoint
        ...
        if (data.status && data.cards) {
            apiData = data.cards;
            document.getElementById('statApiCount').textContent = data.total_apis || apiData.length;
            const totalEndpoints = apiData.reduce((s,c) => s + (c.endpoints?.length || 0), 0);
            document.getElementById('statEndpointCount').textContent = totalEndpoints;
            renderCards(apiData);
            renderAPIsTab(apiData);
            renderDocs(apiData);
            updateDemoSelectors(apiData);
        } else {
            throw new Error('Invalid API response structure');
        }
    } catch(e) { 
        console.error('loadAPIs error:', e); 
        showError(e.message);
        
        // Fallback: Show demo cards if API fails
        const fallbackCards = [
            { id: 'movie', name: 'Movies API', icon: 'fa-film', color: '#6366f1', base_path: '/movie', endpoints: [{ name: 'Search' }] },
            { id: 'game', name: 'Games API', icon: 'fa-gamepad', color: '#10b981', base_path: '/game', endpoints: [{ name: 'Search' }] },
            { id: 'anime', name: 'Anime API', icon: 'fa-dragon', color: '#ec4899', base_path: '/anime', endpoints: [{ name: 'Search' }] },
            { id: 'aone', name: 'AoneRoom API', icon: 'fa-download', color: '#ff8c00', base_path: '/aone', endpoints: [{ name: 'Search' }] }
        ];
        apiData = fallbackCards;
        renderCards(fallbackCards);
        renderAPIsTab(fallbackCards);
        renderDocs(fallbackCards);
        updateDemoSelectors(fallbackCards);
    }
}
...
function renderCards(cards) {
    ...
    container.innerHTML = cards.slice(0,4).map(c => `
        <div class="api-card" onclick="switchToTabAndSelect('demo','${c.id}')">
            <div class="api-icon"><i class="fas ${c.icon || 'fa-code'}" style="color:${c.color || '#6b7280'}"></i></div>
            <h3>${escapeHtml(c.name)}</h3>
            <p>${c.endpoints?.length || 0} endpoints</p>
            <span class="api-badge">${escapeHtml(c.base_path || '/' + c.id)}</span>
        </div>
    `).join('');
}
...
function renderDocs(cards) {
    ...
    container.innerHTML = cards.map(c => `
        <div class="doc-section">
            <h3><i class="fas ${c.icon || 'fa-code'}" style="color:${c.color || '#6b7280'}"></i> ${escapeHtml(c.name)}</h3>
            <p class="doc-description">Base: <code>${escapeHtml(c.base_path || '/' + c.id)}</code></p>
            ${(c.endpoints || []).map(ep => `
                <div class="endpoint-card">
                    <div class="endpoint-header" onclick="toggleEndpoint(this)">
                        <span class="method ${(ep.method || 'GET').toLowerCase()}">${ep.method || 'GET'}</span>
                        <code class="endpoint-path">${escapeHtml(c.base_path || '/' + c.id)}${escapeHtml(ep.path || '')}</code>
                    ...
                    <div class="endpoint-body">
                        ...
                        <h4>🔗 Example:</h4>
                        <code class="example-code">${ep.method || 'GET'} ${escapeHtml(ep.example || (c.base_path || '/' + c.id) + (ep.path || ''))}</code>
...
async function testAPI() {
    ...
    if (!apiSelect || !epSelect || !output) return;
    ...
    let url = `${card.base_path || '/' + card.id}${ep.path || ''}`;
    const params = {};
    ...
    try {
        ...
        output.innerHTML = `<code>Error: ${e.message}\n\nMake sure backend is running on port 3000</code>`; 
    }
}
...
function showError(msg) {
    const container = document.getElementById('apiCardsContainer');
    if(container) container.innerHTML = `<div class="error-message">⚠️ ${escapeHtml(msg)}<br><small>Try: <code>npm run dev</code> and check backend</small></div>`;
    
    const apisContainer = document.getElementById('apisContainer');
    if(apisContainer) apisContainer.innerHTML = `<div class="error-message">⚠️ ${escapeHtml(msg)}</div>`;
}
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            if (epSel && epName) { 
                ...
                for (let i = 0; i < epSel.options.length; i++) {
                    if (epSel.options[i].text.includes(epName) || epSel.options[i].value === epName) {
                        epSel.selectedIndex = i;
                        break;
                    }
                }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Copy button
    const copyIcon = document.querySelector('.response-header i');
    if (copyIcon) copyIcon.addEventListener('click', copyResponse);
